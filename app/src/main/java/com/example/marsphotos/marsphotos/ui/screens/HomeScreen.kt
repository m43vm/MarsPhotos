    ...
    Box(
        ...
        when (photos) {
            is MarsUiState.Success ->
                //Text(text = photos.photos[22].img_src)
                //MarsPhotoCard(photo = photos.photos[1], modifier = modifier.fillMaxSize())
...
fun MarsPhotoCard(photo: MarsPhoto, modifier: Modifier = Modifier) {
    Card(modifier = modifier, elevation = CardDefaults.cardElevation(defaultElevation = 8.dp)) {
        AsyncImage(
            model = ImageRequest.Builder(context = LocalContext.current).data(photo.avatar)
